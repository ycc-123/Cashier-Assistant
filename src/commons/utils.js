...
export async function getWXConfig(url) {
  let wxConfig = {
  ...
  let config = {
    appId: wxConfigRes.data.data.appId,
    timestamp: wxConfigRes.data.data.timestamp,
    nonceStr: wxConfigRes.data.data.nonceStr,
    signature: wxConfigRes.data.data.signature,
  }
  console.log(config)
  return config
}
